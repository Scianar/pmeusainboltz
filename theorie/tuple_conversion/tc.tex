...
Let $\Gamma = (G, M)$ be a context, $R$ a sub-right-rule appearing in $G$ and $t^v, t^b$ tuples such that $\Gamma: R \vdash (t^v, t^b)$. Then $|\{t^{v\bullet} | UnpointTp(t^{v\bullet}, R, False) = t^v \text{ and $t^v$ is left-hand acceptable for $R^{\bullet}$}\}| = |t^v|_z$.
\end{theorem}
