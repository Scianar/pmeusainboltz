...
&&T^{\bullet} &\rightarrow& z \times Set(T) + z \times T^{\bullet} \times Set(T)\\
&\}&
\end{align*}
...
\begin{lstlisting}
#For now ignore the argument left.
def UnpointTp(t: tuple, S: SubRightRule, left: bool) -> tuple:
	switch S:
		case Union(R1, ..., Rn):
			Tp(i, tu) = t #This can raise an error if t has the wrong format.
			return Tp(i, Unpoint(Ri, tu, left))
		case Product(R1, ..., Rn):
			Tp(i, Tp(t1, ..., tn)) = t
			return Tp(t1, ..., Unpoint(Ri, ti, left), ..., tn)
		case RuleName(A):
			if left:
		...
		case Cycle(R) | Set(R):
			Tp(t1, Tp(t2, ..., tn))
			return Tp(Unpoint(R, t1, left), t2, ..., tn)
		case Seq(R):
			Tp(_, Tp(Tp(t_left_1, ..., t_left_n), t_pointed, Tp(t_right_1, ..., t_right_m))) = t
			return Tp(t_left_1, ..., t_left_n, Unpoint(R, t_mid, left), t_right_1, ..., t_right_m)
		case Epsilon() | Marker(_) | Zero():
