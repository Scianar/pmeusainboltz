...
\title{tupLe conversion in usAinboltz for PoINting}
%\title{Tuple conversion for pointing in usainboltz}
...
$$Atom(),\;Epsilon(),\;Marker(name),\;RuleName(name),\;Zero() \text{    for name a character chain.}$$
are sub-right-rules.
\\\\
...
\begin{example}
$R = Product(Atom(), Epsilon())$ is a sub-right-rule.
\\
...
\begin{corollary}
For a context $\Gamma = (G, M)$, $A \rightarrow R \in G$, $t^{b\bullet}$ a tuple. If $t^{b\bullet}$ is a right-hand acceptable tuple for $R^{\bullet}$. Then $M(A)(UnpointTp(t^{b\bullet}, R, True))$ is correctly defined. Therefore $M^{\bullet}(A^{\bullet})$ is correctly defined.
\end{corollary}

\begin{theorem}[UnpointTp respects the size of tuples]
Let $\Gamma = (G, M)$ be a context, $R$ a sub-right-rule appearing in $G$ and $t^{v\bullet}, t^{b\bullet}$ tuples such that $\Gamma^{\bullet}: R^{\bullet} \vdash (t^{v\bullet}, t^{b\bullet})$. Then $UnpointTp(t^{v\bullet}, R, False)|_z = |t^{v\bullet}|_z$.
\end{theorem}

\begin{theorem}[For $n$ tuples pointed of size $n$ there is one non pointed tuple]
Let $\Gamma = (G, M)$ be a context, $R$ a sub-right-rule appearing in $G$ and $t^v, t^b$ tuples such that $\Gamma: R \vdash (t^v, t^b)$. Then $|\{t^{v\bullet} | UnpointTp(t^{v\bullet}, R, False) = t^v \text{ and $t^{v\bullet}$ is left-hand acceptable for $R^{\bullet}$}\}| = |t^v|_z$.
\end{theorem}
...
\begin{itemize}
	\item
$R = Epsilon()$ then $R^{\bullet} = Zero()$, there are no inference rules associated with $Zero()$, therefore there are no $(t^{v\bullet}, t^{b\bullet})$ acceptable for $R$. On the other hand, $UnpointTp(t^{v\bullet}, Epsilon(), False)$ raises an exception.

	\item
$Marker()$ and $Zero()$ are very similar to $Epsilon()$.

	\item
$R = Atom()$ therefore $R^{\bullet} = Atom()$. Then $UnpointTp(t^{v\bullet}, R, False) = t^{v\bullet}$ and $UnpointTp(t^{b\bullet}, R, True) = t^{b\bullet}$ . If $\Gamma^{\bullet}: Atom() \vdash (t^{v\bullet}, t^{b\bullet})$, then we can apply the lemma to directly obtain the direct implication. The inverse implication is identical.
\end{itemize}

Recursive cases, to avoid too much repetition, there is only a proof for unions, and rulenames:
\begin{itemize}
	\item
$R = R_1 + ... + R_n$, then $R^{\bullet} = R_1^{\bullet} + ... + R_n^{\bullet}$. The only inference rule for union gives that $t^{v\bullet} = (i,t'^{v\bullet})$, $t^{b\bullet} = Tp(i,t'^{b\bullet})$ with $1 \le i \le n$ and $\Gamma^{\bullet}: R_i^{\bullet} \vdash (t'^{v\bullet}, t'^{b\bullet})$. $R$ appears in $G$, therefore $R_i$ appears in $G$. By hypothesis, $\Gamma: R_i \vdash (UnpointTp(t'^{v\bullet}, R_i, False), UnpointTp(t'^{b\bullet}, R_i, True))$ we conclude by applying the inference rule for union and simplifying the UnpointTp expression. The inverse implication is identical.
...
$$\Gamma: S \vdash (Unpoint(t^{v\bullet}, S, False), Unpoint(t^{'b\bullet}, S, True))$$

$M(A)(UnpointTp(t^{'b\bullet}, R, True)) = M^{\bullet}(A^{\bullet})(t^{'b\bullet}) = t^{b\bullet} = UnpointTp(t^{b\bullet}, A, True)$. By applying the inference rule for simplification, $$\Gamma: A\vdash (UnpointTp(t^{v\bullet}, A, False), M(A)(UnpointTp(t^{'b\bullet}, R, True))) = (UnpointTp(t^{v\bullet}, A, False), UnpointTp(t^{b\bullet}, A, True))$$
...
\begin{itemize}
	\item
$R = Epsilon()$, as explained above, this situation can't occur.
	\item
The same argument can be used for $Marker(\_)$ and $Zero()$
	\item
$R = Atom()$, therefore $t^{v\bullet} = t^{b\bullet} = Atom()$, and $UnpointTp(., R, _)$ is the identity.
\end{itemize}
Recursive cases (again on union and rulenames).
\begin{itemize}
	\item
$R = R_1 + ... + R_n$, therefore $t^{v\bullet} = Tp(i, t^{'v\bullet})$. By induction, $|UnpointTp(t^{'v\bullet}, R_i, False)|_z = |t{'^v\bullet}|_z$. Knowing that $|(i, t^{'v\bullet})|_z = |t{'v\bullet}|_z$ and $UnpointTp(Tp(i, t^{'v\bullet}), R, False) = UnpointTp(t^{'v\bullet}, R_i, False)$, we have enough to conclude.

	\item
If $R = A$ is a rulename such that $A \rightarrow S \in G$, the corresponding inference rule is the simplification. Moreover $UnpointTp(_, A, True)$ is the identity which is enough to conclude by induction.
\end{itemize}
\end{proof}

\begin{proof}[For $n$ tuples pointed of size $n$ there is one non pointed tuple]
Base cases:
\begin{itemize}
	\item If $R = Epsilon()$ (or $Marker(_)$ or $Zero()$), then $|t^v|_z = 0$ and there are no $t^{v\bullet}$ right-hand acceptable for $R^{\bullet}$, therefore $|\{t^{v\bullet} | UnpointTp(t^{v\bullet}, R, False) = t^v \text{ and $t^{v\bullet}$ is left-hand acceptable for $R^{\bullet}$}\}| = 0$.

	\item If $R = Atom()$, then $t^v = Atom()$ and $|t^v|_z = 1$. There is exactly one right-hand acceptable tuple $t^v{\bullet}$ such that $UnpointTp(t^{v\bullet}, R, False) = t^v$: $t^{v\bullet} = Atom()$. This is enough to conclude.
\end{itemize}
Recursive cases (only for product):
\begin{itemize}
	\item If $R = R_1 \times ... \times R_n$, then $t^v = Tp(t^v_1, ..., t^v_n)$. For any $t^{v\bullet}$ such that $Unpoint(t^{v\bullet}, R^{\bullet}, False) = t^v$, we know that $t^v{\bullet}$ is of the form: $$t^{v\bullet} = Tp(i, Tp(t^v_1, ..., t^v_{i-1}, t^{v\bullet}_i, t^v_{i+1}, ..., t^v_n))$$

	Therefore,
	\begin{align*} \{t^{v\bullet} | UnpointTp(t^{v\bullet}, R, False) = t^v \text{ and $t^{v\bullet}$ is left-hand acceptable for $R^{\bullet}$}\} = \\ \displaystyle \uplus_{i=1}^n \{t^{v\bullet}_i | UnpointTp(t^{v\bullet}_i, R, False) = t^v_i \text{ and $t^{v\bullet}_i$ is left-hand acceptable for $R_i^{\bullet}$}\}
	\end{align*}. We can conlude by applying the hypothesis of induction on each $R_i$.
\end{itemize}
\end{proof}
\end{document}