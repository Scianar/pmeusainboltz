...
The internship was with Martin Pépin and Matthieu Dien as internship supervisors. The goal was to code and integrate pointing to the python library Usainboltz \cite{USAINBOLTZ}. This library implements the Boltzmann method which allows fast uniform sampling on combinatorial class at a fixed size \cite{BS}.
...
The zero class is the class containing no elements.
\end{definition}
...
A specification of a class of combinatorial structures {\rsfs C} is a set of (possibly recursive) equations using only operations described above, atoms, epsilons and zeros such that {\rsfs C} is the unique solution to the primary equation (up to an isomorphism).
...
\begin{itemize}
    ...
    \item
    $(A_1 \times ... \times A_n)^{\bullet} = A_1^{\bullet} \times A_2 \times ... \times A_n + ... + A_1 \times ... \times A_{n-1} \times A_n^{\bullet}$
    \item
...
def GrammarPoint(g: Grammar):
    ...
    for (rulename, rule) in g.rules.items():
        new_rules[Point(rulename)] = Point(rulename)
    g.rules = new_rules
...
The second solution (the one I implemented in Usainboltz) is to derive pointed builders (i.e. builders for pointed class) from the builders provided by the user and the grammar structure. To do so, we need to first define what is an acceptable tuple for a grammar, then we can provide and prove the correctness of our algorithm. This has been done in details in \cite{LAPIN}, we offer here a quick overview of the main idea behind this theory.

To differentiate python tuple from mathematical couple, we use the prefix $Tp$ when noting a python tuple.

\begin{definition}[Acceptable tuple for a grammar]
A context $\Gamma$ is a couple $\Gamma = (G, M)$ where $G$ is a grammar and $M$ is a mapping such that for all rulenames $A$ present in $G$, $\mathcal{B} = M(A)$ is a function which takes as argument, an acceptable tuple for $A$.

For $\Gamma$ a context, $R$ a rule appearing in a production rule of $G$ and $t$ a tuple, we say that $t$ is acceptable for $R$, and we write $\Gamma: R\vdash t$, if and only if this proposition can be obtained from a set of inference rules. Here we only show a couple of them, see \cite{LAPIN} for an exhaustive description of the inference rules.
$$\inference{}{\Gamma: Atom() \vdash Tp(Atom(),)}[Atom]$$

$$\inference{\Gamma: R_1 \vdash t_1\quad ... \quad \Gamma:R_n\vdash t_n}{\Gamma: R_1 \times ... \times R_n \vdash Tp(t_1, ..., t_n)}[Product]$$

$$\inference{\text{$A$ is an auxiliary class associated with the rule $R$ in $G$} \quad \Gamma: R \vdash t}{\Gamma: A \vdash Tp(M(A)(t),)}[Simplification]$$
\end{definition}
...
\begin{definition}[Pointed context]
For a context $\Gamma = (G, M)$, the pointed context $\Gamma^{\bullet}$ is $\Gamma^{\bullet} = (G^{\bullet}, M^{\bullet})$. For $A \rightarrow R$ a production rule of $G$, $M^{\bullet}(A) = M(A)$ and $M^{\bullet}(A^{\bullet})(t^\bullet) = M(A)(UnpointTp(t^{\bullet}, R))$ where $t^{b\bullet}$ is an acceptable tuple for $R^{\bullet}$.
\end{definition}

For this definition to be correct, we need to make sure that $UnpointTp(t^{\bullet}, R)$ is an acceptable tuple for $A$.

\begin{theorem}[Correctness of $UnpointTp$]
Let $\Gamma = (G, M)$ be a context, $R$ a rule appearing in $G$ and $t^{\bullet}$ a tuple.
 
 $\Gamma^{\bullet}:R^{\bullet} \vdash t^{\bullet}$ if and only if $UnpointTp(t^{\bullet}, R)$ is correctly defined and $\Gamma:R \vdash UnpointTp(t^{v\bullet}, R)$.
\end{theorem}

The proof has be done in \cite{LAPIN}.
\section{Cycle Pointing}
\subsection{New unlabelled construction}
Vector pointing is enough for the operations defined above, but 

As mentionned previously, some unlabelled constructions have been omitted. We will now see three new unlabelled constructions:

\begin{definition}[MSet]
If {\rsfs C} is an unlabelled class, then $MSet(\text{\rsfs C})$ is the class of sets of elements of {\rsfs C} where repetitions are allowed.
\end{definition}

\begin{definition}[UCycle]
If {\rsfs C} is an unlabelled class, then $UCycle(\text{\rsfs C})$ is defined identically as for the labelled case (in fact, we could use the same notation $Cyc$).
\end{definition}

\begin{definition}[PSet]
If {\rsfs C} is an unlabelled class, then $Pset(\text{\rsfs C})$ is the class of sets of elements of {\rsfs C} where no repetitions are allowed.
\end{definition}

Unlabelled cycles and power sets were not implemented at first in usainboltz, I also implemented the two during the internship. We will only cover the case for not bounded rules, the probability distribution in the bounded case is completely different.

I want to add a description of the generation for MSET, UCYCLE and PSET in the future.

\subsection{Pointing MSET and UCYCLE}
